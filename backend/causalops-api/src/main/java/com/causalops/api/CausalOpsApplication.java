package com.causalops.api;
import org.springframework.boot.SpringApplication; import org.springframework.boot.autoconfigure.SpringBootApplication; import org.springframework.scheduling.annotation.EnableScheduling;
@SpringBootApplication @EnableScheduling public class CausalOpsApplication { public static void main(String[] args){SpringApplication.run(CausalOpsApplication.class,args);} }
